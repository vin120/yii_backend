<?php
    $this->pageTitle = Yii::t('vcos','攻略图片管理');
...
<?php 
    ...
    if(in_array('225', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('227', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('229', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                    ...
                                    <div class="row">
                                        ...
                                        .restaurant_sel{margin-left:20px;width:250px;}
                                            ...
                                            <div class="list_select_option">
                                             	  <label><?php echo yii::t('vcos', '请选择攻略')?>:</label>
                                             	  <select class='restaurant_sel' name='strategy_all_sel'>
                                             	  <?php $path_url_first = Yii::app()->createUrl('Strategy/strategy_img_list',array('strategy'=> 0,'res'=>$res));?>
                                             	  	<option value="<?php echo $path_url_first;?>" <?php if($str == 0){echo "selected='selected'";}?>><?php echo yii::t('vcos', '全部')?></option>
                                             	  <?php 
                                             	  foreach($strategy_sel as $row){
                                             	  $path_url = Yii::app()->createUrl('Strategy/strategy_img_list',array('strategy'=> $row['strategy_id'],'res'=>$res));?>
                                             	  	<option value="<?php echo $path_url;?>" <?php if($str == $row['strategy_id']){echo "selected='selected'";}?>><?php echo $row['strategy_name']?></option>
                                             	  <?php }?>
                                             	  </select>
                                             	  <label><?php echo yii::t('vcos', '请选择语言')?>:</label>
                                             	  <select class='restaurant_sel' name='lang_all_sel'>
                                             	  <?php $path_url_all = Yii::app()->createUrl('Strategy/strategy_img_list',array('res'=> 'all','strategy'=>$str));?>
                                             	  	<option value="<?php echo $path_url_all;?>" <?php if($res == 'all'){echo "selected='selected'";}?>><?php echo yii::t('vcos', '全部')?></option>
                                             	  <?php $path_url_zh = Yii::app()->createUrl('Strategy/strategy_img_list',array('res'=> 'zh_cn','strategy'=>$str));?>
                                             	  	<option value="<?php echo $path_url_zh;?>" <?php if($res == 'zh_cn'){echo "selected='selected'";}?>><?php echo yii::t('vcos', '中文')?></option>
                                             	  <?php $path_url_en = Yii::app()->createUrl('Strategy/strategy_img_list',array('res'=> 'en','strategy'=>$str));?>
...
jQuery(function($) {
            ...
            buttons: [
                {
                    ...
                    click: function() {
                        location.href="<?php echo Yii::app()->createUrl("Strategy/strategy_img_list");?>"+"?id="+$a;
                        $( this ).dialog( "close" );
                    }
                }
                ,
    ...
    $(".list_select_option select[name='strategy_all_sel']").change(function(){
    	var path = $(this).val();
		window.location.href=path;
	});
