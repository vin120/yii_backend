<?php
    $this->pageTitle = Yii::t('vcos','城市列表');
...
<?php 
    ...
    if(in_array('217', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('219', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('221', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                            ...
                            <div class="row">
                                <div class="col-xs-12"><!-- PAGE CONTENT BEGINS -->
                                    <div class="row">
                                        ...
                                        .country_sel{margin-left:20px;width:250px;}
                                        </style>
                                            <div class="table-responsive">
                                            <div class="list_select_option">
                                             	  <label><?php echo yii::t('vcos', '请选择国家')?>:</label>
                                             	  <select class='country_sel' name='country_all_sel'>
                                             	  	<option value='0' <?php if($res_but == 0){echo 'selected';}?>><?php echo yii::t('vcos', '全部')?></option>
                                             	  <?php foreach($country_sel as $row){
                                             	  	$path_url = Yii::app()->createUrl('Strategy/city_list',array('country'=> $row['country_id']));
                                             	  ?>
                                             	  	<option value="<?php echo $path_url;?>" <?php if($row['country_id'] == $res_but){echo 'selected';}?>><?php echo $row['country_name']?></option>
                                             	  <?php }?>
                                             	  </select>
                                             	</div>
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('vcos', '所属国家')?></th>
                                                            <th><?php echo yii::t('vcos', '城市名')?></th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['city_id'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Strategy/city_list");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
