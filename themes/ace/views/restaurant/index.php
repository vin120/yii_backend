<?php
    $this->pageTitle = Yii::t('vcos','餐厅管理');
    $this->pageTag = 'index';
...
<?php 
    ...
    if(in_array('68', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('69', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('70', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('vcos', '是否支持送餐')?></th>
                                                            <th><?php echo yii::t('vcos', '是否支持预定')?></th>
                                                            <th><?php echo yii::t('vcos', '餐厅分类')?></th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['restaurant_id'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            ...
                                                            <td><?php echo $row['restaurant_opening_time'];?></td>
                                                            <td><?php echo $row['restaurant_tel'];?></td>
                                                            <td><?php echo $row['restaurant_state']?yii::t('vcos', '启用'):yii::t('vcos', '禁用');?></td>
                                                            <td><?php echo $row['can_delivery']?yii::t('vcos', '支持'):yii::t('vcos', '不支持');?></td>
                                                            <td><?php echo $row['can_book']?yii::t('vcos', '支持'):yii::t('vcos', '不支持');?></td>
                                                            <td><?php if($row['restaurant_type']==1){echo yii::t('vcos', '免费餐厅');}else{echo yii::t('vcos', '收费餐厅');}?></td>
                                                            <td><?php echo $row['restaurant_sequence'];?></td>
                                                            <td>
                                                                <?php
                                                                    //操作挂件
                                                                    $this->widget('ManipulateWidget',array(
                                                                        'ControllerName'=>'Restaurant',
                                                                        'MethodName'=>'restaurant_edit',
            ...
            buttons: [
                {
                    ...
                    click: function() {
                        location.href="<?php echo Yii::app()->createUrl("Restaurant/Restaurant_list");?>"+"?id="+$a;
                        $( this ).dialog( "close" );
                    }
                }
                ,
