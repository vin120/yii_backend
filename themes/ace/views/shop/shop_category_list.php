<?php
    $this->pageTitle = Yii::t('vcos','店铺分类列表');
    $theme_url = Yii::app()->theme->baseUrl;
    $menu_type = 'shop_category_list';
?>
<?php 
    //navbar 挂件
    $this->widget('navbarWidget');
    if(in_array('347', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('348', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('349', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                            ...
                                            <div class="table-responsive">
                                            	...
                                            	<div class="list_select_option">
                                             	  ...
                                             	  foreach($shop_sel as $row){
                                             	  	$path_url = Yii::app()->createUrl('Shop/shop_category_list',array('shop'=> $row['shop_id']));
                                             	  ?>
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($shop_category as $key=>$row) { ?>
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['shop_cid'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Shop/shop_category_list");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
