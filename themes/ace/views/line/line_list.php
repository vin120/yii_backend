<?php
    $this->pageTitle = Yii::t('vcos','航线列表');
...
<?php 
    ...
    if(in_array('132', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('133', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('134', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
...
jQuery(function($) {
    ...
    $( ".delete" ).on('click', function(e) {
        var $a = $(this).attr("id");
        e.preventDefault();
        $.post('checkline',{lid:$a},function(data){
            if(data==1){
                $('#danger1').hide();
            ...
            buttons: [
                {
                    ...
                    "id" : "danger1",
                    click: function() {
                        location.href="<?php echo Yii::app()->createUrl("line/line_list");?>"+"?id="+$a;
                        $( this ).dialog( "close" );
                    }
                }
                ,
