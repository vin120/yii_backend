<?php
    $this->pageTitle = Yii::t('vcos','邮轮动态列表');
    $this->pageTag = 'index';
...
<?php 
    ...
    if(in_array('46', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('47', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('48', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('article', '序号')?></th>
                                                            <th><?php echo yii::t('article', '标题')?></th>
                                                            <th><?php echo yii::t('article', '发布时间')?></th>
                                                            <th><?php echo yii::t('article', '状态')?></th>
                                                            <th><?php echo yii::t('article', '开始显示时间')?></th>
                                                            <th><?php echo yii::t('article', '结束显示时间')?></th>
                                                            <th><?php echo yii::t('article', '操作')?></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['article_id'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            <td><?php echo $row['article_title'];?></td>
                                                            <td><?php echo $row['article_time'];?></td>
                                                            <td><?php echo $row['article_state']?yii::t('vcos', '启用'):yii::t('vcos', '禁用');?></td>
                                                            <td><?php echo $row['article_start_time'];?></td>
                                                            <td><?php echo $row['article_end_time'];?></td>
                                                            <td>
                                                                <?php
                                                                    //操作挂件
                                                                    $this->widget('ManipulateWidget',array(
                                                                        'ControllerName'=>'article',
                                                                        'MethodName'=>'article_edit',
                                                                        'id'=>$row['article_id'],
                                                                        'canedit'=>$canedit,
                                                                        'candelete'=>$candelete,
                                                                    ));
    ...
    jQuery(function($) {
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Article/article_list");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
        ...
        $( "#submit" ).on('click', function(e) {
            ...
            if(!$('.isclick').is(':checked')){
                $('#danger').hide();
                $('.widget-header h4').html("<i class='icon-warning-sign red'></i><?php echo yii::t('vcos', '没用选中！')?>");
