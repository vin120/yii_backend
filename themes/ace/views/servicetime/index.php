<?php
    $this->pageTitle = Yii::t('vcos','服务时间列表');
...
<?php 
    ...
    if(in_array('26', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('27', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('28', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('vcos', '部门名称')?></th>
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['service_id'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            <td><?php echo $row['service_department'];?></td>
                                                            <td><?php echo $row['service_tel'];?></td>
                                                            <td><?php echo $row['service_address'];?></td>
                                                            <td><?php echo $row['service_opening_time'];?></td>
                                                            <td><?php echo $row['service_state']?yii::t('vcos', '启用'):yii::t('vcos', '禁用');?></td>
                                                            <td>
                                                                <?php
                                                                    //操作挂件
                                                                    $this->widget('ManipulateWidget',array(
                                                                        'ControllerName'=>'Servicetime',
                                                                        'MethodName'=>'ServiceTime_edit',
                                                                        'id'=>$row['service_id'],
                                                                        'canedit'=>$canedit,
                                                                        'candelete'=>$candelete,
                                                                    ));
            ...
            buttons: [
                {
                    ...
                    click: function() {
                        location.href="<?php echo Yii::app()->createUrl("Servicetime/ServiceTime_list");?>"+"?id="+$a;
                        $( this ).dialog( "close" );
                    }
                }
                ,
