<?php
    $this->pageTitle = Yii::t('vcos','游客评价');
...
<?php 
    ...
    if(in_array('89', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('90', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('91', $auth) || $auth[0] == '0'){
    	$candelete = TRUE;
    }  else {
    	$candelete = FALSE;
    }
                                    ...
                                    <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                            	<td class="center">
                                                   <label>
                                                       <input type="checkbox" name="ids[]" value="<?php echo $row['survey_id'];?>" class="ace isclick" />
                                                       <span class="lbl"></span>
                                                   </label>
                                                </td>
                                                <td><?php echo ++$key; ?></td>
                                                <td><a href="<?php echo Yii::app()->createUrl("Commentandhelp/survey_statistics?id={$row['survey_id']}");?>"><?php echo $row['survey_title']; ?></a></td>
                                                <td><?php echo $row['survey_state']?yii::t('vcos', '启用'):yii::t('vcos', '禁用')?></td>
                                                <td>
                                                    <?php
                                                        //操作挂件
                                                        $this->widget('ManipulateWidget',array(
                                                            'ControllerName'=>'Commentandhelp',
                                                            'MethodName'=>'survey_edit',
                                                            'id'=>$row['survey_id'],
                                                            'canedit'=>$canedit,
                                                        	'candelete'=>$candelete,
                                                        ));
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Commentandhelp/survey");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
