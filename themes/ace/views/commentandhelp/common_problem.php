<?php
    $this->pageTitle = Yii::t('vcos','常见问题');
...
<?php 
    ...
    if(in_array('85', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('86', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('87', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                    ...
                                    <div class="col-xs-12">
                                        <div class="table-responsive">
                                            ...
                                            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                ...
                                                <tbody>
                                                    <?php foreach ($cp as $key=>$row) { ?>
                                                    <tr>
                                                        <td class="center">
                                                            <label>
                                                                <input type="checkbox" name="ids[]" value="<?php echo $row['cm_id'];?>" class="ace isclick" />
                                                                <span class="lbl"></span>
                                                            </label>
                                                        </td>
                                                        <td><?php echo ++$key;?></td>
                                                        <td><?php echo $row['category_name']?></td>
                                                        <td><?php echo $row['cm_title'];?></td>
                                                        <td><?php echo $row['cm_state']? yii::t('vcos', '启用'):yii::t('vcos', '禁用');?></td>
                                                        <td>
                                                            <?php
                                                                //操作挂件
                                                                $this->widget('ManipulateWidget',array(
                                                                    'ControllerName'=>'commentandhelp',
                                                                    'MethodName'=>'commonproblem_edit',
                                                                    'id'=>$row['cm_id'],
                                                                    'canedit'=>$canedit,
                                                                    'candelete'=>$candelete,
                                                                ));
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Commentandhelp/Commonproblem");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
