<?php
    ...
    $menu_type = 'navigation_group_category_list';
?>
<?php 
    //navbar 挂件
    $this->widget('navbarWidget');
    if(in_array('329', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('330', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('331', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                        ...
                                        <div class="col-xs-12">
                                            <div class="table-responsive">
                                            	...
                                            	<div class="list_select_option">
                                             	  <label><?php echo yii::t('vcos', '请选择导航组')?>:</label>
                                             	  <select class='city_sel' name='navigation_group_all_sel'>
                                             	  <?php
                                             	  foreach($navigation_group_sel as $row){
                                             	  	$path_url = Yii::app()->createUrl('Navigation/navigation_group_category_list',array('navigation_group'=> $row['navigation_group_id']));
                                             	  ?>
                                             	  	<option value="<?php echo $path_url;?>" <?php if($row['navigation_group_id'] == $navigation_group_but){echo 'selected';}?>><?php echo $row['navigation_group_name']?></option>
                                             	  <?php }?>
                                             	  </select>
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th><?php echo yii::t('vcos', '导航组')?></th>
                                                            <th><?php echo yii::t('vcos', '导航组分类名')?></th>
                                                            <th><?php echo yii::t('vcos', '排序')?></th>
                                                            <th><?php echo yii::t('vcos', '分类类型')?></th>
                                                            <th><?php echo yii::t('vcos', '是否高亮')?></th>
                                                            <th><?php echo yii::t('vcos', '状态')?></th>
                                                            <th><?php echo yii::t('vcos', '操作')?></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <?php foreach ($navigation_group_category as $key=>$row) { ?>
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['navigation_group_cid'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            <td><?php echo $row['navigation_group_name'];?></td>
                                                            <td><?php echo $row['navigation_category_name'];?></td>
                                                            <td><?php echo $row['sort_order'];?></td>
                                                            <td><?php if($row['category_type']==1){echo '分类';}elseif($row['category_type']==2){echo "品牌";}elseif($row['category_type']==3){echo "店铺";}?></td>
                                                            <td><?php echo $row['is_highlight']?yii::t('vcos', '高亮'):yii::t('vcos', '默认');?></td>
                                                            ...
                                                            <td>
                                                                <?php
                                                                    $this->widget('ManipulateWidget',array(
                                                                        ...
                                                                        'MethodName'=>'navigation_group_category_edit',
                                                                        'id'=>$row['navigation_group_cid'],
                                                                        'canedit'=>$canedit,
                                                                        'candelete'=>$candelete,
                                                                    ));
                                                                ?>
                                                ...
                                                <div class="center">
                                                    <?php
                                                        ...
                                                        $this->widget('BotWidget',array(
                                                            'ControllerName'=>'Navigation',
                                                            'MethodName'=>'navigation_group_category_add',
                                                            'canadd'=>$canadd,
                                                            'candelete'=>$candelete,
                                                        ));
                                                    ?>
...
<script type="text/javascript">
    jQuery(function($) {
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Navigation/navigation_group_category_list");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
        ...
        $(".list_select_option select[name='navigation_group_all_sel']").change(function(){
        	var path = $(this).val();
