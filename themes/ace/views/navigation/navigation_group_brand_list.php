<?php
    ...
    $menu_type = 'navigation_group_brand_list';
?>
<?php 
    //navbar 挂件
    $this->widget('navbarWidget');
    if(in_array('355', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('356', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('357', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                            ...
                                            <div class="table-responsive">
                                            	...
                                            	<div class="list_select_option">
                                             	  ...
                                             	  foreach($navigation_group_sel as $row){
                                             	  	$path_url = Yii::app()->createUrl('Navigation/navigation_group_brand_list',array('navigation_group'=> $row['navigation_group_id']));
                                             	  ?>
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($navigation_group_brand as $key=>$row) { ?>
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['navigation_group_bid'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            ...
                                                            <td>
                                                                <?php
                                                                    $this->widget('ManipulateWidget',array(
                                                                        ...
                                                                        'MethodName'=>'navigation_group_brand_edit',
                                                                        'id'=>$row['navigation_group_bid'],
                                                                        'canedit'=>$canedit,
                                                                        'candelete'=>$candelete,
                                                                    ));
                                                                ?>
                                                ...
                                                <div class="center">
                                                    <?php
                                                        ...
                                                        $this->widget('BotWidget',array(
                                                            'ControllerName'=>'Navigation',
                                                            'MethodName'=>'navigation_group_brand_add',
                                                            'canadd'=>$canadd,
                                                            'candelete'=>$candelete,
                                                        ));
                                                    ?>
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                        	var res = CheckPass();
                            if(res == 1){
                            	location.href="<?php echo Yii::app()->createUrl("Navigation/navigation_group_brand_list");?>"+"?id="+$a;
                            }else if(res == 0){
                                alert('输入密码有误!');
                            }
