<?php
    $this->pageTitle = Yii::t('vcos','活动分类列表');
    $theme_url = Yii::app()->theme->baseUrl;
    $menu_type = 'activity_category_list';
?>
<?php 
    //navbar 挂件
    $this->widget('navbarWidget');
    if(in_array('317', $auth) || $auth[0] == '0'){
        $canadd = TRUE;
    }  else {
        $canadd = False;
    }
    if(in_array('318', $auth) || $auth[0] == '0'){
        $canedit = TRUE;
    }  else {
        $canedit = False;
    }
    if(in_array('319', $auth) || $auth[0] == '0'){
        $candelete = TRUE;
    }  else {
        $candelete = FALSE;
    }
                                            ...
                                            <style>
                                            	...
                                            	<div class="list_select_option">
                                             	  ...
                                             	  foreach($activity_sel as $row){
                                             	  	$path_url = Yii::app()->createUrl('Activity/activity_category_list',array('activity'=> $row['activity_id']));
                                             	  ?>
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($activity_category as $key=>$row) { ?>
                                                        <tr>
                                                            <td class="center">
                                                                <label>
                                                                    <input type="checkbox" name="ids[]" value="<?php echo $row['activity_cid'];?>" class="ace isclick" />
                                                                    <span class="lbl"></span>
                                                                </label>
                                                            </td>
                                                            <td><?php echo ++$key;?></td>
                                                            <td><?php echo $row['activity_name'];?></td>
                                                            <td><?php echo $row['activity_category_name'];?></td>
                                                            ...
                                                            <td>
                                                                <?php
                                                                    $this->widget('ManipulateWidget',array(
                                                                        ...
                                                                        'MethodName'=>'activity_category_edit',
                                                                        'id'=>$row['activity_cid'],
                                                                        'canedit'=>$canedit,
                                                                        'candelete'=>$candelete,
                                                                    ));
                                                ...
                                                <div class="center">
                                                    <?php
                                                        ...
                                                        $this->widget('BotWidget',array(
                                                            'ControllerName'=>'Activity',
                                                            'MethodName'=>'activity_category_add',
                                                            'canadd'=>$canadd,
                                                            'candelete'=>$candelete,
                                                        ));
                                                    ?>
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Activity/activity_category_list");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
