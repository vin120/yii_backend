<?php
    $this->pageTitle = Yii::t('vcos','休闲服务分类管理');
...
<?php 
    ...
    if(in_array('66', $auth) || $auth[0] == '0'){
    	$canadd = TRUE;
    }  else {
    	$canadd = False;
    }
    if(in_array('65', $auth) || $auth[0] == '0'){
    	$canedit = TRUE;
    }  else {
    	$canedit = False;
    }
    if(in_array('273', $auth) || $auth[0] == '0'){
    	$candelete = TRUE;
    }  else {
    	$candelete = FALSE;
    }
                                                ...
                                                <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th width="5%"><?php echo yii::t('vcos', '序号')?></th>
                                                            <th><?php echo yii::t('vcos', '标题')?></th>
                                                            <th><?php echo yii::t('vcos', '图标')?></th>
                                                            <th><?php echo yii::t('vcos', '状态')?></th>
                                                            <th><?php echo yii::t('vcos', '操作')?></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        <?php foreach ($lifeservice_category as $key=>$row) { ?>
                                                        <tr>
                                                        	<td class="center">
			                                                   <label>
			                                                       <input type="checkbox" name="ids[]" value="<?php echo $row['lc_id'];?>" class="ace isclick" />
			                                                       <span class="lbl"></span>
			                                                   </label>
			                                                </td>
                                                            <td><?php echo ++$key;?></td>
                                                            <td><?php echo $row['lc_name'];?></td>
                                                            <td><img src="<?php echo Yii::app()->params['imgurl'].$row['lc_img_url'];?>" style="max-width: 100px"/></td>
                                                            <td><?php echo $row['lc_state']?yii::t('vcos', '启用'):yii::t('vcos', '禁用');?></td>
                                                            <td>
                                                                <?php
                                                                    //操作挂件
                                                                    $this->widget('ManipulateWidget',array(
                                                                        'ControllerName'=>'Lifeservice',
                                                                        'MethodName'=>'Service_categoryedit',
                                                                        'id'=>$row['lc_id'],
                                                                        'canedit'=>$canedit,
                                                                    	'candelete'=>$candelete,
                                                                    ));
                ...
                buttons: [
                    {
                        ...
                        click: function() {
                            location.href="<?php echo Yii::app()->createUrl("Lifeservice/service_category");?>"+"?id="+$a;
                            $( this ).dialog( "close" );
                        }
                    }
                    ,
